...
async function startBackend() {
  if (isDev) {
    console.log("[Electron] Dev mode - backend expected on :8000");
    return;
  }

  const python = findPython();
  if (!python) {
    console.log("[Electron] Python not found");
    return;
  }
  ...
  if (!fs.existsSync(mainPy)) {
    console.log("[Electron] " + mainPy + " not found");
    return;
  }
  ...
  backendProcess.on("exit", (code) => {
    if (code !== 0 && code !== null) console.error("[Electron] Backend exited code", code);
    backendProcess = null;
  });
}
